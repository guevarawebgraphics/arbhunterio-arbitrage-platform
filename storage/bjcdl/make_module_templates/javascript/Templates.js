...
$.template_snake_case.init = {
    activate: function () {
        var token = $('meta[name="csrf-token"]').attr('content');
        var permissions = JSON.parse(window.permissions.split('\\'));
        
        var table = $('#template_snake_case-table').DataTable({
            ajax: '',
            serverSide: true,
            processing: true,
            aaSorting:[[0,"desc"]],
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        method: "POST",
                        dataType: 'JSON',
                        url: sAdminBaseURI + '/template_snake_case_plural/' + rowid +'/delete',
                        success: function (data) {
                            table.row(el.parents('tr'))
                                .remove()
                                .draw();
                            if (permissions.permission.find(perm => perm.name === 'Restore DefaultTemplate')) {
                                Swal.fire({
                                    ...
                                    cancelButtonText: 'Ok'
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            $.ajax({
                                                ...
                                                method: "GET",
                                                url: sAdminBaseURI + '/template_snake_case_plural/' + rowid +'/restore',
                                                success: function (data) {
                                                    $('#template_snake_case-table').DataTable().ajax.reload();
                                                    Swal.fire(
                                                        'Restored!',
...
$(function () {
    $.template_snake_case.init.activate();
});
