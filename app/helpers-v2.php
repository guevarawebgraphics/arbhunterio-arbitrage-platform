...
function getOdds($row) {
    ...
    $best_over_odds_query = getLatestGroupBets([
        'row'   =>  $row,
        'type'  =>  'over'
    ]);

    $best_under_odds_query = getLatestGroupBets([
        'row'   =>  $row,
        'type'  =>  'under'
    ]);

    $best_over_odds_query_array = $best_over_odds_query->toArray();
    $best_under_odds_query_array = $best_under_odds_query->toArray();

    $best_over_odds = !empty($best_over_odds_query_array) 
        ? max(array_column($best_over_odds_query_array, 'bet_price')) 
        : 0.00;

    $best_under_odds = !empty($best_under_odds_query_array) 
        ? max(array_column($best_under_odds_query_array, 'bet_price')) 
        : 0.00;
    ...
    if ( !empty($best_over_odds) || !empty($best_under_odds) ) {
        ...
        $latest_betname_a = $best_over_odds_query;

        $latest_betname_b = $best_under_odds_query;
...
function getLatestGroupBets($data) {
    $gameId = $data['row']['uid'];
    $betType = $data['row']['bet_type'];
    $isLive = 0;
    $isMain = 0;
