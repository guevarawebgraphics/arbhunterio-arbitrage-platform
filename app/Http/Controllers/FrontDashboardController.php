...
class FrontDashboardController extends Controller
{
            ...
            return datatables()->of($games)
                ->addColumn('id', function ($row) {
                    return '<div class="flex items-center">
                            <button data-modal-target="calculatorModal" data-modal-toggle="calculatorModal" class="bg-transparent outline-none" type="button" data-id="'.$row->game_id.'">
    ...
    private function getBestOdds($row) {

        $results = GameOdds::where('game_id', $row->game_id)
            ->where('bet_type', $row->bet_type)
            ->selectRaw('
                MAX(CASE WHEN selection_line = "over" THEN selection_points END) as over_best_odds,
                MAX(CASE WHEN selection_line = "under" THEN selection_points END) as under_best_odds
            ')
            ->first();

        $data = [
            'over'  => $results->over_best_odds ?? 0,
            'under' => $results->under_best_odds ?? 0,
        ];
