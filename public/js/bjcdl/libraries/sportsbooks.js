...
$.sportsbook.init = {
    activate: function () {
        ...
        var table = $('#sportsbook-table').DataTable({
            ajax: '',
            serverSide: true,
            processing: true,
            aaSorting:[[0,"desc"]],
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        method: "POST",
                        dataType: 'JSON',
                        url: sAdminBaseURI + '/sportsbooks/' + rowid +'/delete',
                        success: function (data) {
                            table.row(el.parents('tr'))
                                .remove()
                                .draw();
                            if (permissions.permission.find(perm => perm.name === 'Restore SportsBook')) {
                                Swal.fire({
                                    ...
                                    cancelButtonText: 'Ok'
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            $.ajax({
                                                ...
                                                method: "GET",
                                                url: sAdminBaseURI + '/sportsbooks/' + rowid +'/restore',
                                                success: function (data) {
                                                    $('#sportsbook-table').DataTable().ajax.reload();
                                                    Swal.fire(
                                                        'Restored!',
