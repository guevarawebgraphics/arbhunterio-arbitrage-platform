...
$.coupon_codes.init = {
    activate: function () {
        ...
        var table = $('#coupon_codes-table').DataTable({
            ajax: '',
            serverSide: true,
            processing: true,
            aaSorting:[[0,"desc"]],
            columns: [
                ...
                {data: 'type', name: 'type'},
                {data: 'used', name: 'used'},
                {data: 'coupon_period', name: 'coupon_period'},
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        method: "POST",
                        dataType: 'JSON',
                        url: sAdminBaseURI + '/coupon_codes/' + rowid +'/delete',
                        success: function (data) {
                            table.row(el.parents('tr'))
                                .remove()
                                .draw();
                            if (permissions.permission.find(perm => perm.name === 'Restore Coupon Code')) {
                                Swal.fire({
                                    ...
                                    cancelButtonText: 'Ok'
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            $.ajax({
                                                ...
                                                method: "GET",
                                                url: sAdminBaseURI + '/coupon_codes/' + rowid +'/restore',
                                                success: function (data) {
                                                    $('#coupon_codes-table').DataTable().ajax.reload();
                                                    Swal.fire(
                                                        'Restored!',
