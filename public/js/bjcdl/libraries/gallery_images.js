...
$.gallery_images.init = {
    activate: function () {
        ...
        var table = $('#gallery_images-table').DataTable({
            ajax: '',
            serverSide: true,
            processing: true,
            aaSorting:[[0,"desc"]],
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        method: "POST",
                        dataType: 'JSON',
                        url: sAdminBaseURI + '/gallery_images/' + rowid +'/delete',
                                    ...
                                    cancelButtonText: 'Ok'
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            $.ajax({
                                                ...
                                                method: "GET",
                                                url: sAdminBaseURI + '/gallery_images/' + rowid +'/restore',
                                                success: function (data) {
                                                    $('#gallery_images-table').DataTable().ajax.reload();
                                                    Swal.fire(
                                                        'Restored!',
    ...
    edit: function () {
        ...
        $(function() {
            ...
            if (window.gallery_image != '') {
                $(".image-preview-input-title").text("Change");
                $(".image-preview-clear").show();
                $(".image-preview-filename").val(window.gallery_image); 
                img.attr('src', window.gallery_image);
                $(".image-preview").attr("data-content",$(img)[0].outerHTML).popover("show");
            }
...
$(function () {
    ...
    if (page == 'gallery_images') {
        $.gallery_images.init.activate();
    } else if (page == 'create') {
        $.gallery_images.init.create();
    } else if (page == 'edit') {
        $.gallery_images.init.edit();
    }
});