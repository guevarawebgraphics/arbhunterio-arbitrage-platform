        ...
        <div class="card">
            ...
            <div class="row">
                ...
                <form method="post" action="{{ route('settings.update_profile') }}">
                    @csrf
                    <input type="hidden" name="view" value="bankroll-settings">
                    <div class="p-3 mb-3">
                        <label for="" class="block  text-sm font-medium text-white">Bankroll</label>
                        <input type="number" style="width: 100%;" name="bankroll" value="{{ Request::old('bankroll') ?? auth()->user()->bankroll }}">
                    </div>
                    <div class="p-3 mb-3">
                        <label for="" class="block  text-sm font-medium text-white">Kelly Multiplier</label>
                        <input type="number" style="width: 100%;" name="kelly_multiplier" value="{{ Request::old('bankroll') ?? auth()->user()->bankroll }}">
                    </div>
                    <hr>
                    <br>
